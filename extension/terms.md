...
- **Settings you change:** feature toggles, language/theme, and whether the extension is enabled are saved in Chrome `storage.local`.
...
- **Popup UI preferences:** some UI state (active tab, active downloads sub-tab, sort direction) is stored locally in the popup’s `localStorage`.
...
## Your controls
- You can disable the extension entirely via the popup.
- You can toggle each feature on/off in the popup.
